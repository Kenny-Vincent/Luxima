                    ...
                    if( (has_post_thumbnail() && get_post_format() != 'video') || get_post_format() === false ){
                        ...
                        $output .= '<div class="'.( ($carousel == 'yes')?'swioer-slide':'' ).' blog-item '.esc_attr($style).' '.(($dynamic_from_where == 'one_post')?'single':'').'">'; 
                            if( has_post_thumbnail() )
                                $output .= '<img src="'.esc_url(codeless_image_by_id(get_post_thumbnail_id(), 'port2', 'url')).'" alt="'.esc_attr__('Featured Image','specular').'">';
                            $output .= '<div class="content">';
                                $output .= '<h4><a href="'.esc_url(get_permalink()).'">'.esc_html(get_the_title()).'</a></h4>';
                                $output .= '<ul class="info">';
                                    if(codeless_get_mod('blog_info_author'))
                                        $output .= '<li><i class="linecon-icon-user"></i>'.esc_html__('Posted by','specular').' '.get_the_author().'</li>'; 
                                    if(codeless_get_mod('blog_info_date'))
                                        $output .= '<li><i class="linecon-icon-calendar"></i>'.esc_html__('On','specular').' '.get_the_date().'</li>';   
                                $output .= '</ul>';
                                $output .= '<p>'.codeless_text_limit(get_the_excerpt(), $text_limit).'</p>';

                                if(codeless_get_mod('blog_info_tags') || codeless_get_mod('post_like') ):
                                    $output .= '<div class="after">';

                                        if(codeless_get_mod('blog_info_tags')):
                                            $output .= '<ul class="info">';
                                                $output .= '<li><i class="linecon-icon-tag"></i>'.$tag_out.'</li>';                        
                                            $output .= '</ul>';
                                        endif;

                                        if(codeless_get_mod('post_like'))
                                            $output .= '<div class="post-like">'.codeless_get_post_like_link( get_the_ID() ).'</div>';
                                    $output .= '</div>';
                                endif; 
                            $output .= '</div>'; 
