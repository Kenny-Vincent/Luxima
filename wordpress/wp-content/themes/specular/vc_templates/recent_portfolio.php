...
extract( $atts );
        ...
        if($from_where == 'all_cat'){
            ...
            $query_post = array('posts_per_page'=> $coe, 'post_type'=> 'portfolio',  'taxonomy' => 'portfolio_entries', 'portfolio_entries' => $category_slug );

        }
        

        if( (int) codeless_get_mod('single_portfolio_related') ){
            $category = get_term($category, "portfolio_entries");
            $query_post = array( 
                'tax_query' => array(
                    array(
        ...
        $codeless_used_for_element['style'] = $style;
        $codeless_used_for_element['columns'] = $columns;
        $codeless_used_for_element['carousel'] = $carousel;
