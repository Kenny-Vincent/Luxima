...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
	...
	class Envato_Theme_Setup_Wizard {
		...
		public function init_globals() {
			...
			$this->plugin_path = trailingslashit( $this->cleanFilePath( get_template_directory() . '/envato-setup/' ) );
		...
		public function admin_menus() {
			...
			if( $this->is_submenu_page() ){
				...
				$add_subpage_function( $this->parent_slug, esc_html__( 'Specular Wizard', 'specular' ), esc_html__( 'Specular Wizard', 'specular' ), 'manage_options', $this->page_slug, array( $this, 'setup_wizard' ) );
			}else{
				add_theme_page( esc_html__( 'Specular Wizard', 'specular' ), esc_html__( 'Specular Wizard', 'specular' ), 'manage_options', $this->page_slug, array( $this, 'setup_wizard' ) );
			}
		...
		public function init_wizard_steps() {
			...
			$this->steps = array(
				'introduction' => array(
					'name'    => esc_html__( 'Introduction', 'specular' ),
		...
		public function setup_wizard() {
			...
			wp_localize_script( 'envato-setup', 'envato_setup_params', array(
				...
				'verify_text' => esc_html__( '...verifying', 'specular' ),
			) );
	...
	public function setup_wizard_header() {
		...
		<?php if ( 'next_steps' === $this->step ) : ?>
			<a class="wc-return-to-dashboard" href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard', 'specular' ); ?></a>
		<?php endif; ?>
		</body>
		<?php
...
//				exit;
				...
				?>
				<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.', 'specular' ), wp_get_theme()); ?></h1>
				<p><?php esc_html_e('It looks like you may have recently upgraded to this theme. Great! This setup wizard will help ensure all the default settings are correct. It will also show some information about your new website and support options.','specular');?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!', 'specular' ); ?></a>
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(),'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php esc_html_e( 'Not right now', 'specular' ); ?></a>
				</p>
				<?php
			} else if( get_option('envato_setup_complete', false) ){
				?>
				<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.', 'specular' ), wp_get_theme()); ?></h1>
				<p><?php esc_html_e('It looks like you have already run the setup wizard. Below are some options: ','specular');?></p>
				<ul>
					<li>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button-primary button button-next button-large"><?php esc_html_e( 'Run Setup Wizard Again', 'specular' ); ?></a>
					</li>
					<li>
						<form method="post">
							<input type="hidden" name="reset-font-defaults" value="yes">
							<input type="submit" class="button-primary button button-large button-next" value="<?php esc_attr_e( 'Reset font style and colors', 'specular' ); ?>" name="save_step" />
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(),'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php esc_html_e( 'Cancel', 'specular' ); ?></a>
				</p>
				<?php
			} else {
				?>
				<h1><?php printf( esc_html__( 'Welcome to the setup wizard for %s.', 'specular' ), wp_get_theme()); ?></h1>
				<p><?php printf( esc_html__( 'Thank you for choosing the %s theme from ThemeForest. This quick setup wizard will help you configure your new website. This wizard will install the required WordPress plugins, default content, logo and tell you a little about Help &amp; Support options. <br/>It should only take 5 minutes.' , 'specular' ), wp_get_theme()); ?></p>
				<p><?php esc_html_e( 'No time right now? If you don\'t want to go through the wizard, you can skip and return to the WordPress dashboard. Come back anytime if you change your mind!', 'specular' ); ?></p>

				<h2><?php printf( esc_html__( 'Warning ! Read Me', 'specular' ), wp_get_theme()); ?></h2>
				...
				<ul>
					...
					<li>2. Go to WP Dashboard > Specular > Import/Export   </li>
		...
		public function envato_setup_default_plugins() {
			...
			?>
			<h1><?php esc_html_e( 'Default Plugins', 'specular' ); ?></h1>
			<form method="post">
				...
				if ( count( $plugins['all'] ) ) {
					?>
					<p><?php esc_html_e( 'Your website needs a few essential plugins. The following plugins will be installed or updated:', 'specular' ); ?></p>
					...
					echo '<p><strong>'._e( 'Good news! All plugins are already installed and up to date. Please continue.', 'specular' ).'</strong></p>';
				} ?>

				<p><?php esc_html_e( 'You can add and remove plugins later on from within WordPress.', 'specular' ); ?></p>

				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_plugins"><?php esc_html_e( 'Continue', 'specular' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'specular' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form>
			<?php
		...
		public function ajax_plugins() {
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No Slug Found', 'specular' ) ) );
			}
			$json = array();
			...
			foreach ( $plugins['activate'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2' => -1,
						'message' => esc_html__( 'Activating Plugin', 'specular' ),
					);
					break;
				}
			}
			foreach ( $plugins['update'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2' => -1,
						'message' => esc_html__( 'Updating Plugin', 'specular' ),
					);
					break;
				}
			}
			foreach ( $plugins['install'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2' => -1,
						'message' => esc_html__( 'Installing Plugin', 'specular' ),
					);
					break;
				}
			}
			...
			if ( $json ) {
				...
				wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success', 'specular' ) ) );
			}
			exit;
		...
		private function _content_default_get() {
			...
			foreach($available_content as $post_type => $post_data){
				if(count($post_data)){
					...
					$content[$post_type] = array(
						...
						'description' => sprintf( esc_html__( 'This will create default %s as seen in the demo.', 'specular' ), $post_type_title ),
						'pending' => esc_html__( 'Pending.', 'specular' ),
						'installing' => esc_html__( 'Installing.', 'specular' ),
						'success' => esc_html__( 'Success.', 'specular' ),
			...
			$content['widgets'] = array(
				'title' => esc_html__( 'Widgets', 'specular' ),
				'description' => esc_html__( 'Insert default sidebar widgets as seen in the demo.', 'specular' ),
				'pending' => esc_html__( 'Pending.', 'specular' ),
				'installing' => esc_html__( 'Installing Default Widgets.', 'specular' ),
				'success' => esc_html__( 'Success.', 'specular' ),
			...
			$content['revslider'] = array(
				'title' => esc_html__( 'Revolution Slider', 'specular' ),
				'description' => esc_html__( 'Install Revolution Sliders.', 'specular' ),
				'pending' => esc_html__( 'Pending.', 'specular' ),
				'installing' => esc_html__( 'Installing Revolution Sliders... ', 'specular' ),
				'success' => esc_html__( 'Success.', 'specular' ),
			...
			$content['attachments'] = array(
				'title' => esc_html__( 'Install Images', 'specular' ),
				'description' => esc_html__( 'Install Img and attachments', 'specular' ),
				'pending' => esc_html__( 'Pending.', 'specular' ),
				'installing' => esc_html__( 'Installing attachments.', 'specular' ),
				'success' => esc_html__( 'Success.', 'specular' ),
		    ...
		    $content['demo'] = array(
				'title' => esc_html__( 'Install Content', 'specular' ),
				'description' => esc_html__( 'Install Content of the Demo.', 'specular' ),
				'pending' => esc_html__( 'Pending.', 'specular' ),
				'installing' => esc_html__( 'Installing Demo.', 'specular' ),
				'success' => esc_html__( 'Success.', 'specular' ),
			...
			$content['options'] = array(
				'title' => esc_html__( 'Theme Option', 'specular' ),
				'description' => esc_html__( 'Configure default settings.', 'specular' ),
				'pending' => esc_html__( 'Pending.', 'specular' ),
				'installing' => esc_html__( 'Installing Default Settings.', 'specular' ),
				'success' => esc_html__( 'Success.', 'specular' ),
			...
			$content['menu'] = array(
				'title' => esc_html__( 'Menu', 'specular' ),
				'description' => esc_html__( 'Insert default menu as seen in the demo.', 'specular' ),
				'pending' => esc_html__( 'Pending.', 'specular' ),
				'installing' => esc_html__( 'Installing Default Menu.', 'specular' ),
				'success' => esc_html__( 'Success.', 'specular' ),
		...
		public function envato_setup_default_content() {
			?>
			<h1><?php esc_html_e( 'Choose one demo to be installed', 'specular' ); ?></h1>
			<form method="post">
				<?php if($this->is_possible_upgrade()){ ?>
					<p><?php esc_html_e('It looks like you already have content installed on this website. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.','specular'); ?></p>
				<?php }else{ ?>
					<p><?php printf( esc_html__( 'It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue. It is recommended to leave everything selected. Once inserted, this content can be managed from the WordPress admin dashboard. ', 'specular' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
				<?php } ?>
				...
				<table class="envato-setup-pages" cellspacing="0">
					...
					<tr>
						<td class="check"> </td>
						<th class="item"><?php esc_html_e( 'Item', 'specular' ); ?></th>
						<th class="description"><?php esc_html_e( 'Description', 'specular' ); ?></th>
						<th class="status"><?php esc_html_e( 'Status', 'specular' ); ?></th>
					</tr>
					</thead>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_content"><?php esc_html_e( 'Continue', 'specular' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'specular' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form>
		...
		public function ajax_content() {
			...
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No content Found', 'specular' ) ) );
			}
			...
			if ( $json ) {
				...
				wp_send_json( array( 'error' => 1, 'message' => esc_html__( 'Error', 'specular' ) ) );
			}
...
//									echo "Post meta $meta_key was $meta_val \n\n";
		...
		private function _fetch_remote_file( $url, $post ) {
			...
			if ( !$upload || $upload['error'] ) {
				...
				if ( is_array( $response ) && !empty($response['body']) && $response['response']['code'] == '200' ) {
					...
					@unlink( $upload['file'] );
					return new WP_Error( 'import_file_error', esc_html__( 'Remote server did not respond', 'specular' ) );
				}
				...
				if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
					...
					return new WP_Error( 'import_file_error', esc_html__( 'Remote file is incorrect size', 'specular' ) );
				}

				if ( 0 == $filesize ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', esc_html__( 'Zero size file downloaded', 'specular' ) );
				}

				if ( ! empty( $max_size ) && $filesize > $max_size ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', sprintf( esc_html__( 'Remote file is too large, limit is %s', 'specular' ), size_format( $max_size ) ) );
				}
			}
		...
		private function _content_install_widgets() {
			...
			if($options){
				...
				$msg = '<br />'.esc_html__("Widgets Imported !", 'specular');
				return true;
			}else{
				$msg = '<br />'.esc_html__("Your backup or demo doesn't contain sidebar_widgets.txt file.", 'specular');
				return false;
			}
			  ...
			  if (filter_var($url, FILTER_VALIDATE_URL))
			    {
			        ...
			        $filename = $path;
			        $write = codeless_builder_file_open($filename, 'w');
			 		
			        
			        $buffer = codeless_builder_f_get_contents($url);
			        codeless_builder_f_put_contents($filename, $buffer);

			        codeless_builder_file_close($write);
	    ...
	    public function modifyXml (){
		    ...
		    $path = get_template_directory() . '/includes/dummy_data/demos/'.$dir.'/' . $file.'.xml';
				...
				foreach($items as $item){
					...
					$wp_children->attachment_url = str_replace('http://codeless.co/specular/default/wp-content/uploads/', get_template_directory_uri().'/downloads/uploads/', $wp_children->attachment_url);
		...
		public function _content_install_menu(){
			...
			if($menus_data){
				...
				$this->message .= '<br />'.esc_html__("Menu Imported successfully", 'specular');
				return true;
			}else{
				$this->message .= '<br />'.esc_html__("Your backup or demo doesn't contain sidebar_widgets.txt file.", 'specular');
				return true;
			}
		...
		public function _content_install_revsliders(){
		   ...
		   $dir = $_POST['demo'];	
			...
			foreach ($slider_array as $sliders => $slider_url  ) {
					...
					$uri = 'https://codeless.co/specular/'.$slider_url;
		...
		public function _content_install_options(){
			...
			if($options){
				...
				$options['frontpage'] = $new_['ID'];

				update_option('cl_redata', $options);
				
				$msg = '<br />'.esc_html__("Options imported successfully", 'specular');
				return true;
			}else{
				$msg = '<br />'.esc_html__("Your backup doesn't contain options.txt file.", 'specular');
				return false;
			}
		}
	    ...
	    public function _content_install_demo() {
	    	...
	    	if($dir == 'default')
				...
				require_once( get_template_directory(). '/envato-setup/importer/wordpress-importer.php');
				...
				if(is_wp_error($value)){
					$this->message .= esc_html__("Error during Import. Try Again", 'specular');
					
					return true;
				}
				else {
					$this->message .= esc_html__("Content has been imported successfully ", 'specular');
		...
		private function _get_json( $file, $dir ) {
			if ( is_file( __DIR__.'/content/'.$dir.'/'.basename( $file ) ) ) {
				...
				$file = codeless_decode_content($file_name);
		...
		public function envato_setup_logo_design() {
			 global $cl_redata;
			?>
			<h1><?php esc_html_e( 'Logo &amp; Design', 'specular' ); ?></h1>
			<form method="post">
				<p><?php printf( esc_html__( 'Please add your logo below. For best results, the logo should be a transparent PNG ( 466 by 277 pixels). The logo can be changed at any time from the Appearance > Customize area in your dashboard.','specular'), ''); ?></p>

				<table>
					<tr>
						<td>
							<div id="current-logo">
								<?php $image_url = codeless_get_mod( 'logo', 'url' );
						...
						<td>
							<a href="#" class="button button-upload"><?php esc_html_e( 'Upload New Logo', 'specular' ); ?></a>
						</td>
					</tr>
				</table>
				...
				if( !$this->get_default_theme_style() || !$demo_styles){
					...
					<p><?php esc_html_e( 'Please choose the color scheme for this website. The color scheme (along with font colors &amp; styles) can be changed at any time from the Appearance > Customize area in your dashboard.' , 'specular' ); ?></p>
				...
				<p class="envato-setup-actions step">
					<input type="submit" class="button-primary button button-large button-next" value="<?php esc_attr_e( 'Continue', 'specular' ); ?>" name="save_step" />
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'specular' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form>
		...
		public function envato_setup_logo_design_save() {
			...
			if ( $new_logo_id ) {
				...
				if ( $attr && ! empty( $attr[1] ) && ! empty( $attr[2] ) ) {
					codeless_get_mod('logo')['url'] = $attr[0];
		...
		public function envato_setup_updates() {
			?>
			<h1><?php esc_html_e( 'Theme Updates', 'specular' ); ?></h1>
			<?php if ( function_exists( 'envato_market' ) ) { ?>
				<form method="post">
					...
					if ( count( $my_items ) ) {
						...
						<p class="envato-setup-actions step">
							<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next button-primary"><?php esc_html_e( 'Continue', 'specular' ); ?></a>
						</p>
						<?php
					} else {
						?>
						<p><?php esc_html_e( 'Please login using your ThemeForest account to enable Theme Updates. We update themes when a new feature is added or a bug is fixed. It is highly recommended to enable Theme Updates.', 'specular' ); ?></p>
						<p>When an update becomes available it will show in the Dashboard with an option to install.</p>
						<p>
						...
						<p class="envato-setup-actions step">
							<input type="submit" class="button-primary button button-large button-next" value="<?php esc_attr_e( 'Login with Envato', 'specular' ); ?>" name="save_step" />
							<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php esc_html_e( 'Skip this step', 'specular' ); ?></a>
							<?php wp_nonce_field( 'envato-setup' ); ?>
						</p>
					<?php } ?>
		...
		public function envato_setup_customize() {
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-primary button-large button-next"><?php esc_html_e( 'Continue', 'specular' ); ?></a>
			</p>
		...
		public function envato_setup_help_support() {
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-primary button-large button-next"><?php esc_html_e( 'Agree and Continue', 'specular' ); ?></a>
				<?php wp_nonce_field( 'envato-setup' ); ?>
			</p>
			<?php
		}
		...
		public function envato_setup_ready() {
			...
			<h1><?php esc_html_e( 'Your Website is Ready!', 'specular' ); ?></h1>
			...
			<div class="envato-setup-next-steps">
				<div class="envato-setup-next-steps-first">
					<h2><?php esc_html_e( 'Next Steps', 'specular' ); ?></h2>
					<ul>
						<li class="setup-product"><a class="button button-primary button-large" href="https://twitter.com/codelessthemes" target="_blank"><?php esc_html_e( 'Follow @codelessthemes on Twitter', 'specular' ); ?></a></li>
						<li class="setup-product"><a class="button button-next button-large" href="<?php echo esc_url( home_url('/') ); ?>"><?php esc_html_e( 'View your new website!', 'specular' ); ?></a></li>
					</ul>
				</div>
				<div class="envato-setup-next-steps-last">
					<h2><?php esc_html_e( 'More Resources', 'specular' ); ?></h2>
					<ul>
						<li class="documentation"><a href="<?php echo esc_url("http://codeless.co/documentation")?>" target="_blank"><?php esc_html_e( 'Read the Theme Documentation', 'specular' ); ?></a></li>
						<li class="howto"><a href="https://wordpress.org/support/" target="_blank"><?php esc_html_e( 'Learn how to use WordPress', 'specular' ); ?></a></li>
						<li class="rating"><a href="<?php echo esc_url("http://themeforest.net/downloads") ?>" target="_blank"><?php esc_html_e( 'Leave an Item Rating', 'specular' ); ?></a></li>
						<li class="support"><a href="<?php echo esc_url("http://codeless.co")?>" target="_blank"><?php esc_html_e( 'Get Help and Support', 'specular' ); ?></a></li>
					</ul>
				</div>
			</div>
			<?php
		...
		public function envato_market_admin_init() {
			...
			if ( ! empty( $_POST['oauth_session'] ) && ! empty( $_POST['bounce_nonce'] ) && wp_verify_nonce( $_POST['bounce_nonce'], 'envato_oauth_bounce_' . $this->envato_username ) ) {
				...
				if ( is_wp_error( $response ) ) {
					...
					$class = 'error';
					echo "<div class=\"$class\"><p>".sprintf( esc_html__( 'Something went wrong while trying to retrieve oauth token: %s', 'specular' ), $error_message ).'</p></div>';
				} else {
			...
			add_settings_section(
				...
				sprintf( esc_html__( 'Login for %s updates', 'specular' ), $this->envato_username ),
