...

	{
...

		{
...

			{
				...
				wp_enqueue_style(  'codeless_custom_menu', get_template_directory_uri() . '/includes/core/css/codeless_custom_menu.css'); 

				wp_enqueue_script( 'codeless_custom_menu' , get_template_directory_uri() . '/includes/core/js/codeless_custom_menu.js',array('jquery', 'jquery-ui-sortable'), false, true ); 
...

	{
		...
		function start_el(&$output, $item, $depth = 0, $args = array() , $current_object_id = 0) {
			...
			if ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
				...
				$title = sprintf( '%s'.esc_html__(' (Pending)', 'specular'), $item->title );
			...
			<li id="menu-item-<?php echo esc_attr($item_id); ?>" class="<?php echo esc_attr($itemValue); echo implode(' ', $classes ); ?>">
				...
				<dl class="menu-item-bar">
					...
					<dt class="menu-item-handle">
						...
						<span class="item-controls">
							...
							<span class="item-type item-type-column"><?php esc_html_e('Mega Menu Column', 'specular'); ?></span>

							<span class="item-type item-type-megalabel"><?php esc_html_e('Mega Menu', 'specular'); ?></span>

							

							<a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_html_e('Edit Menu Item', 'specular'); ?>" href="<?php

								echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );

							?>"><?php esc_html_e( 'Edit Menu Item' , 'specular'); ?></a>
				...
				<div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
					...
					<?php if( 'custom' == $item->type ) : ?>
						...
						<p class="field-url description description-wide">
							...
							<label for="edit-menu-item-url-<?php echo esc_attr($item_id); ?>">
								...
								<?php esc_html_e( 'URL', 'specular' ); ?><br />
					...
					<p class="description description-thin description-label label_desc_on_active">
						...
						<span class='default_label'><?php esc_html_e( 'Navigation Label', 'specular' ); ?></span>

						<span class='mega_label'><?php esc_html_e( 'Title for the Mega Menu Column', 'specular' ); ?></span>
					...
					<p class="description description-thin description-title">
						...
						<label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
							...
							<?php esc_html_e( 'Title Attribute', 'specular' ); ?><br />
					...
					<p class="field-link-target description description-thin">
						...
						<label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
							...
							<?php esc_html_e( 'link Target', 'specular' ); ?><br />
							...
							<select id="edit-menu-item-target-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-target" name="menu-item-target[<?php echo esc_attr($item_id); ?>]">
								...
								<option value="" <?php selected( $item->target, ''); ?>><?php esc_html_e('Same window or tab', 'specular'); ?></option>

								<option value="_blank" <?php selected( $item->target, '_blank'); ?>><?php esc_html_e('New window or tab', 'specular'); ?></option>
					...
					<p class="field-css-classes description description-thin">
						...
						<label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
							...
							<?php esc_html_e( 'CSS Classes (optional)' , 'specular'); ?><br />
					...
					<p class="field-xfn description description-thin">
						...
						<label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
							...
							<?php esc_html_e( 'link Relationship (XFN)', 'specular' ); ?><br />
					...
					<p class="field-description description description-wide">
						...
						<label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
							...
							<?php esc_html_e( 'Description', 'specular' ); ?><br />
					...
					<div class='codeless_custom_menu_options'>
						...
						<p class="description description-wide codeless_checkbox codeless_custom_mega_menu codeless_custom_mega_menu_d0">
							...
							<label for="edit-<?php echo esc_attr($key.'-'.$item_id); ?>">
								...
								<input type="checkbox" value="active" id="edit-<?php echo esc_attr($key.'-'.$item_id); ?>" class=" <?php echo esc_attr($key); ?>" name="<?php echo esc_attr( $key ) . "[". $item_id ."]";?>" <?php echo codeless_complex_esc($value); ?> /><?php echo esc_attr( $title ); ?>
					...
					<div class="menu-item-actions description-wide submitbox">
						...
						<?php if( 'custom' != $item->type ) : ?>
							...
							<p class="link-to-original">
								...
								<?php printf( esc_html__('Original: %1$s', 'specular'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
						...
						); ?>"><?php esc_html_e('Remove', 'specular'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php	echo add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) );

							?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel', 'specular'); ?></a>
