    ...
    class ReduxFramework_codeless_import {
        ...
        function render() {
            ...
            echo '<div id="import_export_default_section_group' . '">';
	            ...
	            echo '<h2>' . __( 'Import Options', 'codeless' ) . '</h2>';
	            echo '<div class="redux-section-desc">';
	            	echo '<p class="description">' . __( 'You can import all demo data or only a piece of that. Select one from demo data or select a backup to import', 'codeless' )  . '</p>';
	            echo '</div>';
	            echo '<div class="opt_field">';
	            	echo '<span class="title"></span>';
	            	echo '<select id="import_type"><option value="all">All</option><option value="content">Content</option><option value="options">Theme Options</option><option value="widgets">Widgets</option><option value="on_builder">Import From Online Builder</option></select>';
	            echo '</div>';

	            echo '<div class="opt_field">';
	            	echo '<span class="title">'.__('Check to import attachments', 'codeless').'</span>';
	            	echo '<input type="checkbox" value="1" id="attachments" />';
	            echo '</div>';

                echo '<div class="opt_field online_builder">';

                    echo '<h4>Import From Online Builder</h4>';
                    echo '<textarea id="online_builder"></textarea>';
            ...
            echo '<p><a href="#" id="import-data" class="button-primary">Import</a><span>' . apply_filters( 'redux-import-warning', __( 'WARNING! This will overwrite all existing option values, please proceed with caution!', 'codeless' ) ) . '</span></p>';
            ...
            echo '<h2>' . __( 'Export Options', 'codeless' ) . '</h2>';
            echo '<div class="redux-section-desc">';
            	echo '<p class="description">' . __( 'Here you can download your current site data. Keep this safe as you can use it as a backup should anything go wrong, or you can use it to restore your settings on this site (or any other site). You can send your dummy data to Codeless Team too if you want to share with the community', 'codeless' ) . '</p>';
            echo '</div>';

            echo '<div class="opt_field">';
	            	echo '<span class="title"></span>';
	            	echo '<select id="import_type2"><option value="all">All</option><option value="content">Content</option><option value="options">Theme Options</option><option value="widgets">Widgets</option></select>';
	        echo '</div>';

	       

            echo '<script type="text/javascript">var secret = "'.$secret.'";</script>';
            echo '<p><a href="#" id="save-backup" class="button-primary">' . __( 'Save as backup', 'codeless' ) . '</a> <a href="#" id="send-codeless" class="button-secondary">' . __( 'Send to community', 'codeless' ) . '</a></p>';
        ...
        public function enqueue() {
            wp_enqueue_script(
                'codeless-redux-field-import-export-js', 
                cl_path_url('include/redux/inc/fields/codeless_import/codeless_import.js'), 
                array('jquery'), 
                time(), 
                true
            );
            ...
            wp_enqueue_style(
                'codeless-redux-field-import-export-css', 
                cl_path_url('include/redux/inc/fields/codeless_import/codeless_import.css'), 
                time(), 
                true
            ); 
        } 
